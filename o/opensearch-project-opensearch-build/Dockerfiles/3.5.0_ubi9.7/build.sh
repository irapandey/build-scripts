...
#   2. Checkout opensearch-build (tag 3.5.0) and build-scripts (opensearch-3.5.0-ais)
...
#   - opensearch-3.5.0-linux-ppc64le.tar.gz        (assembled dist)
...
#                                  5=container-image 6=scan 7=push
#                          Example: --resume-from 5
# =============================================================================
...
OPENSEARCH_BUILD_BRANCH="3.5.0"   # checked out as a tag

BUILD_SCRIPTS_REPO="https://github.com/irapandey/build-scripts.git"
BUILD_SCRIPTS_BRANCH="opensearch-3.5.0-ais"
...
PATCH_FILE="o/opensearch-project-opensearch-build/Dockerfiles/3.5.0_ubi9.7/ppc64le-3.5.0-ai-services.patch"
DOCKERFILE_DIR="o/opensearch-project-opensearch-build/Dockerfiles/3.5.0_ubi9.7"

# Build-env image name
BUILD_ENV_IMAGE="opensearch-build-env:3.5.0-ppc64le"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: $0 [--skip-push] [--skip-scan] [--resume-from <1|2|3|4|5|6|7>]" >&2
            exit 1
            ;;
    esac
done
...
_validate_resume_from() {
    if [[ -z "${_RESUME_IDX}" ]]; then
        ...
        [[ "${_RESUME_IDX}" -ge 0 ]] || \
            die "Invalid --resume-from value '${RESUME_FROM}'. Valid: 1 2 3 4 5 6 7"
    fi
}
...
if stage_should_run 4; then
    ...
    # Apply all shell/python changes to the opensearch-build clone.
    ...
    podman run --rm \
        ...
        bash -c 'set -euo pipefail && \
            ./build.sh \
                manifests/3.5.0/opensearch-3.5.0.yml \
                --platform '"${PLATFORM}"' \
...
if stage_should_run 5; then
    ...
    # Copy extra plugin zips (if any exist) into the Docker context
    PLUGIN_ZIPS_DIR="${DOCKER_CONTEXT}/plugin-zips"
    mkdir -p "${PLUGIN_ZIPS_DIR}"
    if [[ -d "${ARTIFACTS_DIR}/plugin-zips" ]]; then
        cp -v "${ARTIFACTS_DIR}/plugin-zips/"*.zip "${PLUGIN_ZIPS_DIR}/" 2>/dev/null || true
    fi
