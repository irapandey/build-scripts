...
yum install -y git make wget python$PYTHON_VERSION python$PYTHON_VERSION-devel python$PYTHON_VERSION-pip pkgconfig atlas libjpeg-devel openblas-devel pkgconf-pkg-config
...
export LD_LIBRARY_PATH=$CURRENT_DIR/pytorch/torch/lib64/libprotobuf.so.3.13.0.0:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=$CURRENT_DIR/pytorch/build/lib/libprotobuf.so.3.13.0.0:$LD_LIBRARY_PATH
export PATH="$CURRENT_DIR/protobuf/local/libprotobuf/bin/protoc:${PATH}"
